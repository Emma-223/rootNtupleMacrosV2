...
bkgNames = ['DYJets', 'TTbar']
nominalBkgPlotNames = ['Mee_BkgControlRegion_DYJets', 'Mee_BkgControlRegion_gteTwoBtaggedJets_TTbar']

for idx, bkgName in enumerate(bkgNames):
    nominalBkgPlotName = nominalBkgPlotNames[idx]
    ...
    for var in varList:
      ...
      #lineDown.Draw()
      lowerRightTTBar = [0.55,0.19,0.83,0.40]
      lowerRightDYJ = [0.55,0.14,0.88,0.40]
      upperLeftTTBar = [0.19,0.7,0.51,0.89]
      upperLeftDYJ = [0.19,0.65,0.56,0.89]
      upperRightTTBar = [0.5,0.72,0.81,0.89]
      upperRightDYJ = [0.5,0.67,0.88,0.89]
      if eejjMode:
        if bkgName.lower() == 'ttbar':
          if var=='sT':
            leg = TLegend(*upperRightTTBar)
            graph.GetYaxis().SetRangeUser(-0.05, 1.5)
          elif var=='MejMin':
            leg = TLegend(*upperLeftTTBar)
            graph.GetYaxis().SetRangeUser(0.5, 3.0)
          elif var=='NJetEq':
            leg = TLegend(*upperLeftTTBar)
            graph.GetYaxis().SetRangeUser(0.5, 3.0)
          else:
          ...
          if var=='sT':
            ...
            leg = TLegend(0.19,0.19,0.53,0.40)
